...
public:
...

    {
       ...
       /*
       ros::Publisher pub = nh.advertise<std_msgs::Float32>("please", 1000);
       ros::Subscriber sub1 = nh.subscribe ("/kinect2/sd/image_ir_rect", 1000, &SubscribeAndPublish::cloud_cb1,this);
  	   ros::Subscriber sub2 = nh.subscribe ("/kinect2/sd/image_depth_rect", 1000, &SubscribeAndPublish::cloud_cb2,this);
       ros::Subscriber sub3 = nh.subscribe ("/kinect2/sd/camera_info", 1000, &SubscribeAndPublish::cloud_cb3,this);
       ros::Subscriber sub4 = nh.subscribe ("/kinect2/sd/camera_info", 1000, &SubscribeAndPublish::cloud_cb4,this);
       ros::Subscriber sub5 = nh.subscribe ("invoke_visualizer", 1000, &SubscribeAndPublish::cloudViewer,this);
       ros::spin ();
...
*/
		...
		void cloudViewer(const std_msgs::Float32ConstPtr& number)
		{
   			...
   			if(a==0)
  			 {
  				...
  				cylinder();
   				...
   				visualizer->addPointCloud(cloud_filtered_fin, cloudName);
   ...
   //visualizer->addLine<pcl::PointXYZRGB> (cloud_filtered_fin->points[0], cloud_filtered_fin->points[cloud_filtered_fin->size() - 1], "line");
  				a=a+1;
   				...
   				visualizer->updatePointCloud(cloud_filtered_fin, cloudName);
